 ...
 *
 * The Python implementation carries the same text and CI diffs the two, because a tool that
 * reports drift should not ship documentation that has drifted from its twin.
 */

import { TOOL_VERSION } from "./report.js";

const DOCS = "https://github.com/aton-of-data/retrieval-eval";

export const COMMANDS = ["drift", "score", "validate", "convert"] as const;
export type Command = (typeof COMMANDS)[number];

export const ROOT_HELP = `retrieval-eval ${TOOL_VERSION}
...
const DRIFT_HELP = `retrieval-eval drift
...
const SCORE_HELP = `retrieval-eval score
...
const VALIDATE_HELP = `retrieval-eval validate
...
const CONVERT_HELP = `retrieval-eval convert
