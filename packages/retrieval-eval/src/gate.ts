...
function parseNumber(value: string, expression: string): number {
  ...
  if (Number.isNaN(parsed)) throw new Error(`gate '${expression}': '${value}' is not a number`);
  return parsed;
}
...
export function evaluateGates({ report, baseline, gates }: EvaluateGatesOptions): {
  ...
  for (const gate of gates) {
    ...
    if (result.status === "FAIL" || result.status === "INDETERMINATE") {
      reasons.push(describe(gate, result));
    }
  }
...
function evaluateGate(gate: Gate, report: Report, baseline?: Report): GateResult {
  if (gate.kind === "worst-stratum") {
    ...
    for (const [name, stratum] of Object.entries(report.per_stratum)) {
      const measurement = stratum.metrics[gate.metric];
      if (!measurement) continue;
      if (!worst || measurement.value < worst.value) worst = { name, value: measurement.value };
    }
...
function describe(gate: Gate, result: GateResult): string {
  ...
  if (gate.kind === "delta") {
    ...
    return `${gate.metric} fell ${(from - to).toFixed(4)}, from ${from.toFixed(4)} to ${to.toFixed(4)}`;
  }
  if (gate.kind === "worst-stratum") {
